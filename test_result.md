...
user_problem_statement: "Пользователь сообщает о проблеме с AI рекрутером в Telegram Mini App: при запуске инструмента 'поиск работы' AI-рекрутер выдает ошибку 'Ошибка запуска AI рекрутера'. Это отдельная версия приложения с авторизацией через Telegram (не веб-версия с Google OAuth)."
